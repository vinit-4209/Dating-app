...
const DEMO_EMAIL_VERIFICATION = process.env.DEMO_EMAIL_VERIFICATION === "true"; //for demo purposes only
...
app.post("/api/auth/signup", async (req, res) => {
  try {
    ...
    let token, hashedToken, verificationExpires;

    if (!DEMO_EMAIL_VERIFICATION) {
      const verificationData = generateVerificationToken();
      token = verificationData.token;
      hashedToken = verificationData.hashedToken;
    ...
    // await user.save();
    user.password = hashedPassword;
user.verified = DEMO_EMAIL_VERIFICATION ? true : false;
user.verificationToken = DEMO_EMAIL_VERIFICATION ? undefined : hashedToken;
user.verificationExpires = DEMO_EMAIL_VERIFICATION ? undefined : verificationExpires;
...
await user.save();
  ...
  await sendVerificationEmail({
    to: email,
    subject: 'Verify your LoveConnect account',
    html: `<p>Welcome to LoveConnect, ${name}!</p>
    ...
    //   });
    res.status(200).json({
  message: DEMO_EMAIL_VERIFICATION
    ? 'Signup successful (demo mode). Email verification skipped.'
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    // }
    if (!user.verified && !DEMO_EMAIL_VERIFICATION) {
  return res.status(401).json({ message: 'Please verify your email before logging in.' });
    ...
    res
      .status(500)
      .json({ message: "Unable to log in. Please try again later." });
  }
});
...
app.get("/api/match", requireAuth, async (req, res) => {
  try {
    ...
    const partnerIds = matches.map((m) =>
      m.participants.find((p) => String(p) !== String(myId))
    );
    const profiles = await Profile.find({ userId: { $in: partnerIds } }).lean();
    const profileMap = profiles.reduce(
      (acc, p) => ({ ...acc, [String(p.userId)]: p }),
      {}
    );

    const hydrated = matches.map((match) => {
      const partnerId = match.participants.find(
      ...
      return {
        ...match,
        with: partnerProfile
          ? { ...partnerProfile, id: partnerProfile.userId }
          : null,
      };
    });
...
app.post("/api/messages/:matchId", requireAuth, async (req, res) => {
  try {
    ...
    if (
      !match ||
      match.status !== "accepted" ||
      !match.participants.includes(req.user.id)
    ) {
      return res
        .status(400)
        .json({ message: "Chat is locked until both sides accept." });
    }
